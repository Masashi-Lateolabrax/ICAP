...
class CMAESwM(BaseOptimizer):
    def __init__(self, d, discrete_space, weight_func, minimization_problem:bool, m=None, C=None, sigma=1., minimal_eigenval=1e-30,
    ...
    def update(self, X, evals):
        ...
        if flag_count and (flag_not_improve or flag_min_std_pc or flag_equal_hist or flag_noeffectaxis or flag_noeffectcoord or flag_conditioncov):
            ...
            if isinstance(self.weight_func, CMAWeight):
                self.weight_func = CMAWeight(self.lam, min_problem=self.minimization_problem)
            else:
                self.weight_func = CMAWeightWithNegativeWeights(self.lam, self.d, min_problem=self.minimization_problem)
            # CMA parameters
            self.mu_eff = CMAParam.mu_eff(self.lam)
