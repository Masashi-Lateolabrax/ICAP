...
class BaseWorldBuilder:
    ...
    def build(self) -> tuple[World, dict]:
        ...
        dummies_dict = {o.builder_name: o.gen_dummy_geom() for o in self._objs}
        dummies_list = [] if len(dummies_dict) == 0 else [d for d_list in dummies_dict.values() for d in d_list]

        self.generator.add_children([self.asset, self.world_body, self.actuator, self.sensor])
        xml = self.generator.build()
        model = mujoco.MjModel.from_xml_string(xml)
        world = World._create(model, dummies_list, timer)

        objects = {o.builder_name: o.extract(model, world.data, dummies_dict[o.builder_name], timer) for o in self._objs}
