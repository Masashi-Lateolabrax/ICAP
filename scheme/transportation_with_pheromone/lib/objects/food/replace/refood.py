...
def _replace_food(width: float, height: float, food: Food, invalid_area: list[np.ndarray], padding: float = 0):
...
class ReFood:
    ...
    def update(self):
        ...
        for vf in food_iter:
            ...
            for n in self._nest:
                in_nest = np.linalg.norm(vf.position - n.position) < n.size
                if in_nest:
                    new_vf = vf.copy()
