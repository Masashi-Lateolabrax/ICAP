...
from scheme.transportation_with_pheromone.lib.objects.food import ReFood
from scheme.transportation_with_pheromone.lib.objects.nest import NestBuilder, Nest
...
class Task(optimizer.MjcTaskInterface):
    def __init__(self, world: World, nest: Nest, robot: Robot, refood: ReFood):
        self.time_length: int = Settings.SIMULATION_TIME_LENGTH
...
class TaskGenerator(optimizer.TaskGenerator):
    ...
    def generate(self, para, debug=False) -> Task:
        ...
        return Task(world, nest, robot, refood)
