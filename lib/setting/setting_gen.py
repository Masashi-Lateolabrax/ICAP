...
def _save(settings: dict, external_funcs: list[_Exec], setting_file_path: str):
    working_directory = os.path.dirname(
        os.path.abspath(setting_file_path)
    )

    output_file_path = os.path.join(working_directory, "settings.py")
...
def generate(setting_file_path: str):
    settings, external_funcs = _parser(setting_file_path)
    _save(settings, external_funcs, setting_file_path)
